...
export async function generateStaticParams() {
  return SITE_CONFIG.routes.german_brands.map((brand) => ({
    brand: brand.label,
  }));
}

export async function generateMetadata({ params }: { params: { brand: string } }): Promise<Metadata> {
  const brandLabel = params.brand;
  const brand = SITE_CONFIG.routes.german_brands.find(b => b.label === brandLabel);
...
export default function GermanBrandPage({ params }: { params: { brand: string } }) {
  const brandLabel = params.brand;
  const brand = SITE_CONFIG.routes.german_brands.find(b => b.label === brandLabel);
